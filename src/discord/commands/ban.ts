  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      ...
      const user = await User.findOne({
        username_lower: interaction.options
          .getString("username")
          ?.toLowerCase(),
      });
      if (!user) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            "Reason: Couldn't find the user you were trying to ban."
          );
        ...
        const embed = new EmbedBuilder()
          ...
          .setTitle("Failed to ban user")
          .setDescription(
            "Reason: The user you were trying to ban is allready banned!"
          );
      ...
      if (process.env.BAN_EMBED_ENABLED === "true") {
        ...
        if (typeof channelId === "string") {
          ...
          if (banChannel && banChannel.isTextBased()) {
            ...
            console.log(
              `Could not find text-based channel with the ID: ${channelId}`
            );
          }
        }
      }
